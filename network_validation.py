...
def find_path(filename: str, topology: str) -> str:
    """ Returns current examples directory, if not found programm exits """
...
def create_interfaces() -> None:
    """ Creates the network interface objects from graph's edge attributes 
        ...
        TODO: Maybe Use Factory design pattern to refactor some code
    """
...
def find_subnets() -> tuple:
    """ Finds physical and router subnets in the network 
        and adds them as an attribute to the network graph object
    """
    ...
    def find_physical_subnets():
        ...
        -A physical subnet is a subnet that the devices connected at it need no router
        to communicate.
    ...
    def create_networks_subnet_table() -> None:
        """ Create a table (pandas dataframe) that contains every subnet address in 
...
def validate_mac(subnets: list) -> bool:
    """ Checks if there are any duplicate mac addresses in a given subnet"""
    
    is_valid_mac = True
    
    def is_unique_subnet_mac():
        """ Checks if every mac address in a subnet is unique"""
...
def is_valid_ip_subnet(subnet: nx.DiGraph) -> bool:
    """ Validates the ip subnet is correctly configurated"""
...
def main(topology: str) -> None:
    ...
    # for node in NETWORK_GRAPH.nodes:
    #     if NETWORK_GRAPH.nodes[node]['node_type'] == 'Router':
    #         print(f"Router: {node}, connections: {NETWORK_GRAPH.nodes[node]['connections']}")
    # Phony routing table validation message
    #print("Routing table validation completed!\n")
